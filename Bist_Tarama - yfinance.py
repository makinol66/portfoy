...
def analiz_motoru(hisseler, macd_aktif, hacim_aktif, ema200_aktif, ichi_aktif, nwe_aktif, temel_aktif, teknik_aktif, fibo_p, secilen_t):
    ...
    eur_kur = 35.0  
    if temel_aktif:
        durum_metni.info("⏳ Güncel döviz kurları kontrol ediliyor...")
        try:
            usd_df = yf.download("TRY=X", period="1d", progress=False)
            if not usd_df.empty: usd_kur = float(usd_df['Close'].squeeze().iloc[-1])
            eur_df = yf.download("EURTRY=X", period="1d", progress=False)
            if not eur_df.empty: eur_kur = float(eur_df['Close'].squeeze().iloc[-1])
        except: pass

    durum_metni.info(f"🚀 {toplam_hisse} varlığın son 10 yıllık fiyat geçmişi TEK SEFERDE indiriliyor... (Lütfen bekleyin)")
    try:
        if toplam_hisse > 1:
            toplu_veri = yf.download(hisseler, period="10y", interval="1d", auto_adjust=True, threads=True, progress=False)
        else:
            toplu_veri = yf.download(hisseler[0], period="10y", interval="1d", auto_adjust=True, progress=False)
    except Exception as e:
        st.error(f"⚠️ Fiyatlar toplu olarak çekilirken hata oluştu: {e}")
        return []

    durum_metni.info("✅ Fiyatlar başarıyla indirildi! Formüller ve skorlar hesaplanıyor...")
    ...
    for i, hisse in enumerate(hisseler):
        if temel_aktif:
            time.sleep(0.2)
            if i > 0 and i % 50 == 0:
                durum_metni.warning(f"🛡️ Yahoo engeline takılmamak için 10 saniye mola veriliyor... ({i}/{toplam_hisse})")
                time.sleep(10)
            durum_metni.info(f"🏢 Bilanço ve Temel Analiz verileri çekiliyor: {hisse} ({i+1}/{toplam_hisse})")
        else:
            if i % 10 == 0:
                durum_metni.info(f"⚡ Teknik formüller işleniyor... ({i+1}/{toplam_hisse})")
        
        try:
            if toplam_hisse > 1:
                close = toplu_veri['Close'][hisse].squeeze().dropna()
                try: high = toplu_veri['High'][hisse].squeeze().dropna()
                except: high = pd.Series(dtype=float)
                try: low = toplu_veri['Low'][hisse].squeeze().dropna()
                except: low = pd.Series(dtype=float)
                try: hacim_data = toplu_veri['Volume'][hisse].squeeze().dropna()
                except: hacim_data = pd.Series(dtype=float)
            else:
            ...
            if close.empty or len(close) < 3: 
                ...
                if hisse_kodu in endeks_listesi:
                    try:
                        ...
                        durum_metni.info(f"🔄 {hisse_kodu} verisi eksik, TradingView'den çekiliyor...")
                        tv_data = tv.get_hist(symbol=hisse_kodu, exchange='BIST', interval=Interval.in_daily, n_bars=2500)
            ...
            satir_verisi = {
                ...
                'Zirveden': round(((fyt - zirve_fyt) / zirve_fyt) * 100, 2),
    ...
    if temel_aktif and sonuclar:
        durum_metni.info("⏳ Sektörel medyanlar hesaplanıyor ve adil puanlama yapılıyor...")
        df_temp = pd.DataFrame(sonuclar)
        
        medyan_fk = df_temp[df_temp['F/K'] > 0].groupby('Sektör')['F/K'].median()
        medyan_pd = df_temp[df_temp['PD/DD'] > 0].groupby('Sektör')['PD/DD'].median()
...
with ust_orta:
    ...
    secilen_tarih = st.date_input("Özel Tarih Getirisi Başlangıcı:", date(2023, 9, 7))
    ...
    if cb1.button("✅ Tümünü Seç", use_container_width=True):
        for k in ['macd_k', 'ema_k', 'hacim_k', 'ichi_k', 'nwe_k', 'temel_k', 'tek_k']: 
...
if analiz_baslat:
    ...
    try:
        ...
        if hisseler:
            res = analiz_motoru(hisseler, macd_istiyor_mu, hacim_istiyor_mu, ema200_istiyor_mu, ichimoku_istiyor_mu, nwe_istiyor_mu, temel_istiyor_mu, teknik_istiyor_mu, secilen_fibo_periyot, secilen_tarih)
            if res: 
...
if "tablo_verisi" in st.session_state and st.session_state["tablo_verisi"] is not None:
    ...
    if not temel_istiyor_mu:
        for c in ['Sektör', 'Temel Skor', 'F/K', 'PD/DD', 'ROE (%)']:
    ...
    basa_alinacaklar = ['Hisse', 'Sektör', 'Günlük (%)', 'Fiyat', 'Destek', 'Direnç', 'Hedef (%)', 'Stop (%)']
    ...
    for s in df.columns:
        ...
        if chk_key not in st.session_state:
            if chk_key in kayitli_sutun_ayarlari:
                ...
                st.session_state[chk_key] = False if s in ["Seçili Tarihten", "1.5Y", "2Y", "3Y", "5Y", "TÜM", "EMA 200", "Hacim", "F/K", "PD/DD", "ROE (%)"] else True
    ...
    if sec_cols:
        ...
        for g_sutun in ["Zirveden", "Seçili Tarihten", "1A", "3A", "6A", "1Y", "2Y", "3Y", "TÜM"]:
            col_ayarlari[g_sutun] = st.column_config.NumberColumn(format="%.2f %%")
        ...
        if st.session_state.get("png_goster", False):
            ...
            for col in df_png.columns:
                ...
                elif col in ["Fiyat", "Destek", "Direnç"] or "EMA" in col:
                    df_png[col] = df_png[col].apply(lambda x: formatla(x, "{:.2f}"))
                elif "RSI" in col:
                    df_png[col] = df_png[col].apply(lambda x: formatla(x, "{:.1f}"))
                elif "Günlük" in col or "Hedef" in col or "Stop" in col or "Pot." in col or "Bollinger" in col or "Seçili" in col or "Zirve" in col or "1A" in col or "TÜM" in col or "ROE" in col:

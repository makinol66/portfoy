...
def portfoydeki_hisseleri_getir():
    ...
    cuzdanlar = db_sync.load_data("portfoy", {})
...
def analiz_motoru(hisseler, macd_aktif, hacim_aktif, ema200_aktif, ichi_aktif, nwe_aktif, temel_aktif, teknik_aktif, zirve_aktif, fibo_p, secilen_t, veri_kaynagi):
    ...
    for i, hisse in enumerate(hisseler):
        ...
        try:
            ...
            # --- STANDART ZİRVEDEN SÜTUNU HESAPLAMASI ---
            zirve_fyt_standart = float(close.max())
            zirveden_oran = ((fyt - zirve_fyt_standart) / zirve_fyt_standart) * 100 if zirve_fyt_standart > 0 else 0.0
            
            # --- ATH ZİRVE (GÜN & UZAKLIK) HESAPLAMALARI ---
            _zirve_gun = 0
            _zirve_yuzde = 0.0
            if zirve_aktif:
            ...
            satir_verisi = {
                ...
                'Fiyat': fyt, 
                'Zirveden': round(zirveden_oran, 2),
...
with ust_sol:
    ...
    st.markdown("<p style='font-size: 14px; font-weight: 600; margin-bottom: 5px; color: #31333F;'>📌 Ana Havuzlar</p>", unsafe_allow_html=True)
    c_havuz1, c_havuz2, c_havuz3 = st.columns(3)
    with c_havuz1:
        t_tum = st.toggle("BIST TÜM", value=True)
        t_100 = st.toggle("BIST 100")
        t_500 = st.toggle("BIST 500")
        t_katilim = st.toggle("KATILIM TÜM")
    with c_havuz2:
        t_50 = st.toggle("BIST 50")
        t_30 = st.toggle("BIST 30")
        t_fav = st.toggle("Favoriler")
        t_port = st.toggle("Portföy")
    with c_havuz3:
        t_man = st.toggle("Manuel Giriş")
        t_end = st.toggle("Endeks (Kıyas)")
        t_tem = st.toggle("BIST TEMETTÜ")
        t_surd = st.toggle("SÜRDÜRÜLEBİLİR")
        
    secim = []
    if t_tum: secim.append("BIST TÜM")
    if t_100: secim.append("BIST 100")
    if t_500: secim.append("BIST 500")
    if t_katilim: secim.append("BIST KATILIM TÜM")
    if t_50: secim.append("BIST 50")
    if t_30: secim.append("BIST 30")
    if t_tem: secim.append("BIST TEMETTÜ")
    if t_surd: secim.append("BIST SÜRDÜRÜLEBİLİRLİK")
    if t_fav: secim.append("Favori Hisseler")
    if t_port: secim.append("Portföydekiler")
    if t_man: secim.append("Manuel Giriş")
    if t_end: secim.append("Endekslerin Kendisi (Kıyaslama)")
    
    st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
    sektorel_secim = st.multiselect(
        "🏢 Sektörel Havuzlar (Açılır Liste):",
        ("BIST BANKA", "BIST SINAİ", "BIST MALİ", "BIST HİZMETLER", "BIST TEKNOLOJİ", 
         ...
         "BIST KATILIM 100", "BIST KATILIM 50", "BIST KATILIM 30")
    )
    secim.extend(sektorel_secim)
...
if "tablo_verisi" in st.session_state and st.session_state["tablo_verisi"] is not None:
    st.session_state.favoriler = db_sync.load_data("favoriler", {})
    ...
    basa_alinacaklar = ['Hisse', 'Sektör', 'Günlük (%)', 'Fiyat', 'Zirveden', 'Zirveye Uzaklık (%)', 'Zirve (Gün)', 'Destek', 'Direnç', 'Hedef (%)', 'Stop (%)']
    ...
    if sec_cols:
        ...
        if "Hisse" in df_gosterim.columns:
            endeks_kisa_adlari = ["XUTUM", "XU500", "XU100", "XU050", "XU030", "XBANK", "XUSIN", "XUMAL", "XUHIZ", "XUTEK", "XGIDA", "XHOLD", "XILTM", "XINSA", "XMADN", "XSPOR", "XTRZM", "XTCRT", "XULAS", "XKTUM", "XK100", "XK050", "XK030", "XTMTU", "XSRD"]
            
            def link_olustur(x):
                kisa_kod = x.replace('.IS', '')
                if kisa_kod in endeks_kisa_adlari:
                    return f"https://fintables.com/endeksler/{kisa_kod}"
                return f"https://fintables.com/sirketler/{kisa_kod}"
                
            df_gosterim["Hisse"] = df_f["Hisse"].apply(link_olustur)
        ...
        col_ayarlari = {
            ...
            "Hisse": st.column_config.LinkColumn("Hisse", display_text=r"https://fintables.com/(?:sirketler|endeksler)/(.*)", disabled=True),
            ...
            "Zirveden": st.column_config.NumberColumn("Zirveden", format="%.2f %%"),
        ...
        if st.session_state.get("png_goster", False):
            ...
            for col in df_png.columns:
                ...
                elif "Günlük" in col or "Hedef" in col or "Stop" in col or "Pot." in col or "Bollinger" in col or "Seçili" in col or ("Zirve" in col and "%" in col) or "Zirveden" in col or "1A" in col or "TÜM" in col or "ROE" in col:
